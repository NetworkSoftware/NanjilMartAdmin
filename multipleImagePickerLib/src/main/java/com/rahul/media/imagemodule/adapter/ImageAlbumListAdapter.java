    ...
    private void loadImage(String thumbPath, final SquareImageView imgAlbum) {
        ...
        if (imageByte != null) {
            ...
            if (thumbnail != null) {
                ...
                Glide.with(mContext)
                        .applyDefaultRequestOptions(new RequestOptions().centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL))
                        .load(thumbnail)
                        .into(imgAlbum);
            } else {
                Glide.with(mContext)
                        .applyDefaultRequestOptions(new RequestOptions()
                                .centerCrop().diskCacheStrategy(DiskCacheStrategy.ALL).override(200, 200))
                        // this cropping technique scales the image so that it fills the requested bounds and then crops the extra.
                        .asBitmap()
